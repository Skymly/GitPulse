...
public class PullRequestsViewModelTests
{
    ...
    public async Task Load_WithUnauthorizedResponse_SetsErrorMessage()
    {
        ...
        // ListPullRequestsPaged is ApiResponse<PullRequest[]>; non-2xx is Page Error (M-05).
    ...
    public async Task StateFilter_ChangeWhileLoadMore_ReloadsFromPage1()
    {
        ...
        var handler = new MockHttpHandler()
            .When("/repos/owner/repo/pulls", req =>
            {
                ...
                if (i == 1)
                    return new MockResponse(PrsJson(("open", false, false)), LinkHasNext);
                if (i == 2)
                    return new MockResponse(PrsJson(("open", false, false)), LinkNoNext, Gate: more.Task);
                return new MockResponse(
                    $"[{GitHubJson.PullRequest(999, state: "closed")}]",
                    LinkNoNext);
            });
        var factory = new FakeGitHubClientFactory(handler);
        var vm = new PullRequestsViewModel(factory);
        ...
        Assert.Single(vm.PullRequests);
        Assert.Equal(999, vm.PullRequests[0].Number);
        Assert.False(vm.IsLoading.Value);
        vm.Dispose();
    }
}
