...
> **版本**：Unreleased
> **关联 ADR**：[ADR-002](../adr/ADR-002-observables-declarative-github-api.md)、[ADR-018](../adr/ADR-018-per-request-pagination-options.md)、[ADR-008](../adr/ADR-008-split-github-search-api-interface.md)、[ADR-009](../adr/ADR-009-split-github-actions-api-interface.md)
...
- `q` 在 `IGitHubSearchApi` 上显式声明；`page` / `per_page` 由 session 写入逐请求 `HttpRequestOptions`，`GitHubQueryHandler` 只读 options 注入 query，并从 `Link` 头判断下一页。
...
4. Search 的 `q` 必须保留在声明式接口签名中；业务查询参数走 `[Query]`。分页 `page` / `per_page` / `state` 走逐请求 `HttpRequestOptions`，由无状态 `GitHubQueryHandler` 注入。
...
2. Session 把 `page` / `per_page`（及 Issues/PRs 的 `state`）写入该次请求的 `HttpRequestOptions`；无状态 `GitHubQueryHandler` 只读 options 注入 query。Handler 不是 ViewModel 面向契约
